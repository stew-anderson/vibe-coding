...
import { ALLOWED_CATEGORIES, isAllowedCategory } from "@/lib/categories"
...
export default function Home() {
  ...
  const loadLinks = async (categoryFilter?: string) => {
    ...
    try {
      ...
      if (categoryFilter && isAllowedCategory(categoryFilter)) params.set("category", categoryFilter)
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!url.trim() || !isAllowedCategory(category)) return
    try {
      const res = await fetch(`/api/links`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: url.trim(), title: title.trim() || undefined, category })
      })
  ...
  return (
    <main style={{ padding: 24, maxWidth: 720, margin: "0 auto" }}>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 8, marginBottom: 24 }}>
        ...
        <button type="submit" style={{ padding: 10 }} disabled={!isAllowedCategory(category)}>Submit</button>
      </form>
      ...
      <ul style={{ listStyle: "none", padding: 0, display: "grid", gap: 8 }}>
        {links.map((l) => (
          <li key={l.id} style={{ border: "1px solid #e5e5e5", padding: 12, borderRadius: 8 }}>
            ...
            <div style={{ fontSize: 12, color: "#666" }}>Category: {l.category}</div>
          </li>
        ))}
      </ul>
    </main>
